<template>
  <v-dialog v-model="localIsDialogOpen" max-width="600px">
    <base-card>
      ...
      <template #default>
        <v-form ref="form">
          <v-file-input v-model="file" variant="outlined" density="compact" label="File" prepend-icon="mdi-paperclip"
            :rules="[rules.required]" required></v-file-input>
          <v-text-field v-if="showModelPoint" v-model="fileName" density="compact" variant="outlined" :label="mpLabel"
            :rules="[rules.required]" required></v-text-field>

          <v-select v-model="year" variant="outlined" density="compact" :label="yearLabel" :items="years"
...
const emit = defineEmits<{
  (e: 'upload', payload: { file: File | null; fileName: string; productCode: string; year: number | null }): void;
