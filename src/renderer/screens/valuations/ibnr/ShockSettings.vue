    ...
    <v-row>
      <v-col>
        <base-card :showActions="false">
          <template #header>
            <span class="headline">IBNR Shock Settings</span>
          </template>
          <template #default>
            <v-container>
              <v-row>
...
onMounted(() => {
  IbnrService.getAvailableShockBases().then((res) => {
    shockBases.value = res.data
    console.log(shockBases.value)
  })
  IbnrService.getShockSettings().then((response) => {
...
const deleteShockSetting = async (scenario: any) => {
  ...
  try {
    ...
    if (result) {
      ...
      const response = await IbnrService.deleteShockSetting(scenario.id)
...
const addToScenarios = () => {
  ...
  IbnrService.saveShockSetting(setting).then((res) => {
