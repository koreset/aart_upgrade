  ...
  <v-container fluid>
        ...
        <base-card>
          ...
          <template #default>
            <v-container>
              <v-row>
                <v-col cols="4">
                  <v-select v-model="settingWorkflow" variant="outlined" density="compact" label="Setting workflow"
                    placeholder="Choose a workflow" :items="settingWorkflows" item-title="name" item-value="type"
                    @update:model-value="displayFlow"></v-select>
                </v-col>
              </v-row>
              <v-row v-if="settingWorkflow == 'existing'">
                <v-col cols="4">
                  <v-select v-model="selectedTemplate" variant="outlined" density="compact"
                    label="Choose a saved template" :items="jobTemplates" item-title="name" item-value="id" clearable
                    return-object @update:model-value="loadTemplate"></v-select>
                </v-col>
                <v-col v-if="selectedTemplate" class="d-flex" cols="6">
                  <v-text-field v-model="runNameSuffix" label="Run Name Suffix"
                    placeholder="Add a suffix to existing run name" density="compact" variant="outlined"></v-text-field>
                  <v-btn rounded class="mt-2 ml-5" color="primary" size="small" @click="updateRunSuffix">Update Run
                    Name</v-btn>
                </v-col>
              </v-row>
              <v-row v-if="settingWorkflow == 'new'">
                <v-col cols="6">
                  <v-text-field v-model="settingRunName" :error-messages="errors.settingRunName"
                    v-bind="settingRunNameAttrs" variant="outlined" density="compact"
              ...
              <v-row v-if="settingWorkflow == 'new'">
                <v-col>
                  <v-textarea v-model="settingDescription" variant="outlined" rows="3"
                    placeholder="Enter a description for this run"></v-textarea>
                </v-col>
              </v-row>
              <v-row v-if="settingWorkflow == 'new'">
                <v-col>
                  <v-combobox v-model="selectedProducts" v-bind="setSelectedProducts" variant="outlined"
                    density="compact" label="Available Products" :items="availableProducts" :clearable="true"
                    placeholder="Choose products to run" multiple chips closable-chips return-object
                    item-title="product_name" @update:model-value="validateProduct"></v-combobox>
                </v-col>
              </v-row>
              <v-row v-if="settingWorkflow == 'new'">
                <v-col cols="4">
                  <v-select v-model="selectedModelPointYear" v-bind="setSelectedModelPointYear" density="compact"
                    variant="outlined" label="Select Model Point Year" :items="availableModelPointYears"
                    item-title="Year" item-value="Year" @update:model-value="getModelPointVersions"></v-select>
                </v-col>
                <v-col cols="4">
                  <v-select v-model="selectedModelPointVersion" density="compact" variant="outlined"
                    label="Select Model Point Version" :items="availableModelPointVersions" item-text="Year"
                    item-value="Year"></v-select>
                </v-col>

                <v-col cols="4">
                  <v-select v-model="selectedYieldYear" density="compact" variant="outlined"
                    label="Select Yield Curve Year" :items="availableYieldYears" item-title="Year"
                    item-value="Year"></v-select>
                </v-col>
                <v-col cols="4">
                  <v-select v-model="selectedParameterYear" density="compact" variant="outlined"
                    label="Select Parameter Year" :items="availableParameterYears" item-title="Year" item-value="Year"
                    @update:model-value="getRunBasis"></v-select>
                </v-col>
                <v-col cols="4">
                  <v-select v-model="selectedMorbidityYear" density="compact" variant="outlined"
                    label="Select Morbidity Year" :items="availableMorbidityYears" item-title="Year"
                    item-value="Year"></v-select>
                </v-col>
                <v-col cols="4">
                  <v-select v-model="selectedRetrenchmentYear" density="compact" variant="outlined"
                    label="Select Retrenchment Year" :items="availableRetrenchmentYears" item-title="Year"
                    item-value="Year"></v-select>
                </v-col>
                <v-col cols="4">
                  <v-select v-model="selectedLapseYear" density="compact" variant="outlined" label="Select Lapse Year"
                    :items="availableLapseYears" item-title="Year" item-value="Year"></v-select>
                </v-col>
                <v-col cols="4">
                  <v-select v-model="selectedMortalityYear" density="compact" variant="outlined"
                    label="Select Mortality Year" :items="availableMortalityYears" item-title="Year"
                    item-value="Year"></v-select>
                </v-col>
                <v-col cols="4">
                  <v-select v-model="selectedShock" density="compact" variant="outlined" label="Select Applicable Shock"
                    :items="shockData" item-title="name" item-value="name" return-object></v-select>
                </v-col>
                <v-col cols="4">
                  <v-select v-model="selectedYieldCurveBasis" density="compact" variant="outlined"
                    label="Select Yield Curve Basis" :items="availableYieldCurveBases" item-title="basis"
                    item-value="basis"></v-select>
                </v-col>
                <v-col cols="4">
                  <v-select v-model="selectedLapseMarginYear" density="compact" variant="outlined"
                    label="Select Lapse Margin Years" :items="availableLapseMarginYears" item-title="Year"
                    item-value="Year"></v-select>
                </v-col>
                <v-col cols="4"><v-text-field v-model="aggPeriod" density="compact" variant="outlined" type="number"
                    label="Aggregation Period" placeholder="Enter Aggregation period in months"></v-text-field>
                </v-col>
                <v-col cols="4"><v-text-field v-model="yearEndMonth" density="compact" variant="outlined" type="number"
                    max="12" min="1" label="Year End Month" placeholder="Enter Year End Month (1 - 12)"></v-text-field>
                </v-col>
                <v-col cols="4">
                  <v-select v-model="selectedBasis" density="compact" variant="outlined" label="Run Basis"
                    placeholder="Select a Run Basis" :items="availableBases" item-title="name"
              ...
              <v-row v-if="settingWorkflow == 'new'">
                ...
                <v-col cols="3">
                  <v-checkbox v-model="ifrs17Indicator" :label="`IFRS17 Indicator`"
                    @change="toggleIndicators('ifrs17')"></v-checkbox></v-col>
                <v-col cols="3">
                  <v-checkbox v-model="licIndicator" :label="`LIC Indicator`"
                    @change="toggleIndicators('lic')"></v-checkbox></v-col>
                <v-col cols="3">
                  <v-checkbox v-model="noIndicator" :label="`Use No Indicators`"
                    @change="toggleIndicators('none')"></v-checkbox></v-col>
              </v-row>
              <v-row v-if="settingWorkflow == 'new'">
                <v-col>
                  <v-btn rounded color="primary" size="small" class="primary" @click="addToRunJobs">Add to
                    Run
                    Jobs</v-btn>
                </v-col>
              </v-row>
                  ...
                  <v-table class="trans-tables">
                    <thead>
                      <tr class="table-row">
                        ...
                        <th class="text-left table-col">Model Points <v-btn v-if="settingWorkflow === 'existing'"
                            size="small" variant="plain" icon @click="openEditDialog('modelpoint_year')">
                            <v-icon>mdi-pencil</v-icon>
                          </v-btn></th>
                        <th class="text-left table-col">Parameters <v-btn v-if="settingWorkflow === 'existing'"
                            size="small" variant="plain" icon @click="openEditDialog('parameter_year')">
                            <v-icon>mdi-pencil</v-icon>
                          </v-btn></th>
                        <th class="text-left table-col">Yield Curve <v-btn v-if="settingWorkflow === 'existing'"
                            size="small" variant="plain" icon @click="openEditDialog('yieldcurve_year')">
                            <v-icon>mdi-pencil</v-icon>
                          </v-btn></th>
                        <th class="text-left table-col">Lapse <v-btn v-if="settingWorkflow === 'existing'" size="small"
                            variant="plain" icon @click="openEditDialog('lapse_year')">
                            <v-icon>mdi-pencil</v-icon>
                          </v-btn></th>
                        <th class="text-left table-col">Lapse Margin <v-btn v-if="settingWorkflow === 'existing'"
                            size="small" variant="plain" icon @click="openEditDialog('lapse_margin_year')">
                            <v-icon>mdi-pencil</v-icon>
                          </v-btn></th>
                        <th class="text-left table-col">Mortality <v-btn v-if="settingWorkflow === 'existing'"
                            size="small" variant="plain" icon @click="openEditDialog('mortality_year')">
                            <v-icon>mdi-pencil</v-icon>
                          </v-btn></th>
                        <th class="text-left table-col">Morbidity <v-btn v-if="settingWorkflow === 'existing'"
                            size="small" variant="plain" icon @click="openEditDialog('morbidity_year')">
                            <v-icon>mdi-pencil</v-icon>
                          </v-btn></th>
                    ...
                    item.shock_settings !== null
                      ...
                      : "N/A"
                        ...
                        <td>
                          <v-btn color="red" size="small" variant="plain" icon
          ...
          <template #actions>
            <v-btn v-if="runJobs.length > 0" color="primary" @click="saveTemplate">Save as template</v-btn>
            <v-btn v-if="runJobs.length > 0" color="primary" @click="executeJobs">Run Valuations</v-btn>
          </template>
        </base-card>
      </v-col>
    </v-row>
    ...
    <v-dialog v-model="dialog" max-width="500px">
      <v-card>
        ...
        <v-card-text>
          <v-text-field v-model="newColumnValue" variant="outlined" density="compact" label="New Value"></v-text-field>
        </v-card-text>
        <v-card-actions>
...
const loadTemplate = async () => {
  const resp = await ValuationService.getJobTemplate(
...
const getModelPointVersions = async () => {
  if (selectedModelPointYear.value !== null) {
    ...
    ValuationService.getModelPointVersionsForProduct(selectedProducts.value[0].id, selectedModelPointYear.value).then((resp) => {
      if (resp.data !== null) {
    ...
    if (availableBases.value.length > 0) {
      ...
      if (intersection.length < 1) {
        returnValidationError(
          "There are differences in the available bases"
        );
        return;
      }
...
const validateProduct = async () => {
  if (selectedProducts.value.length === 0) {
    ...
    returnValidationError(
      "You need to provide model points for this product to run"
    );
    return;
  }
  try {
    if (selectedProducts.value.length > 0) {
      ...
      if (res.data.results !== null && res.data.results < 1) {
        returnValidationError(
          "There are no model points available for this product"
        );
        return;
      }
      ...
      if (availableParameterYears.value.length > 0) {
        ...
        if (intersection.length < 1) {
          returnValidationError(
            "There are differences in the available parameter points in years"
          );
          return;
        }
