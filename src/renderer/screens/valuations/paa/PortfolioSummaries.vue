      ...
      <v-col>
        <base-card>
          ...
          <template #default>
            <v-row>
              <v-col cols="3">
                <v-select v-model="selectedPortfolio" label="Choose a Portfolio" variant="outlined" density="compact"
                  :items="portfolios" item-text="name" item-title="name" return-object
                  @update:model-value="getModelPointYears"></v-select>
              </v-col>
              <v-col cols="3">
                <v-select v-model="selectedModelPointYear" label="Select a Model point year" variant="outlined"
                  density="compact" :items="modelPointYears" item-title="year" item-value="year"
                  @update:model-value="getModelPointVersions"></v-select>
              </v-col>
              <v-col cols="3">
                <v-select v-model="selectedModelPointVersion" label="Select a Model point Version" variant="outlined"
...
const getModelPointVersions = () => {
  if (selectedModelPointYear.value) {
    ModifiedGMMService.getAvailableMpVersions(selectedPortfolio.value.name, selectedModelPointYear.value).then(res => {
