...
const handleUpload = (event: any, item: any) => {
  ...
  ModifiedGMMService.uploadGmmModelpointTables(formData)
    .then((res) => {
      const yearVersion = res.data.year_version
      console.log(yearVersion)
      loadDataComplete.value = true
      // update the portfolio with the new year_versions
      const portfolio = paginatedPortfolios.value.find((elem) => {
        return elem.id === item.id
      })
      console.log('Portfolio found: ', portfolio)
      if (portfolio) {
        portfolio.year_versions.push(yearVersion)
      }
    })
    .catch((err) => {
      console.log(err)
