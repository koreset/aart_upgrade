        ...
        <base-card>
          ...
          <template #default>
            <v-row class="mt-9 mx-5">
              <v-col cols="3">
                <v-text-field v-model="portfolioName" class="mr-9" placeholder="Portfolio Name" label="Portfolio Name"
                  variant="outlined" density="compact" :error-messages="portfolioNameErrors"></v-text-field>
              </v-col>
              <v-col cols="3">
                <v-select v-model="discountOption" placeholder="Select a Discount Option" label="Discount Option"
                  variant="outlined" density="compact" :items="discountTypes" item-title="text"
                  item-value="value"></v-select>
              </v-col>
              <v-col cols="3">
                <v-select v-model="premiumEarningPattern" placeholder="Select a Premium Earning Pattern"
                  label="Premium Earning Pattern" variant="outlined" density="compact" :items="earningPatterns"
                  item-title="text" item-value="value"></v-select>
              </v-col>
              <v-col cols="3">
                <v-select v-model="selectedInsuranceType" placeholder="Select an Insurance Type" label="Insurance Type"
                  variant="outlined" density="compact" :items="insuranceTypes" item-title="text"
                  item-value="value"></v-select>
              </v-col>
              <v-col class="d-flex align-baseline" cols="3">
                <v-btn rounded class="primary mt-1" small @click="createPortfolio">Add Portfolio</v-btn>
              </v-col>
            </v-row>
                ...
                <base-card :showActions="false">
                  ...
                  <template #default>
                    <v-expansion-panels v-model="activePanel">
                      <v-expansion-panel v-for="item in portfolios" :key="item.name" @group:selected="checkClass">
                                ...
                                <v-fade-transition leave-absolute>
                                  <v-row v-if="expanded" no-gutters style="width: 100%">
                                    <v-col class="d-flex justify-end">
                                      <v-btn class="ml-7 pb-2 mr-9" variant="text" icon
                        ...
                        <v-expansion-panel-text>
                          <v-row class="mt-5">
                            ...
                            <v-col>
                              <b>Insurance Type: </b> {{ item.insurance_type }}
                            </v-col>
                          </v-row>
                          <v-row> </v-row>
                          <v-row> </v-row>
                          <v-row>
                            <v-col>
                              ...
                              <v-row v-if="item.year_versions == null ||
                  item.year_versions.length == 0
                  ">
                                <v-col>
                                  There are no model points uploaded for this
                                  portfolio
                                </v-col>
                              </v-row>
